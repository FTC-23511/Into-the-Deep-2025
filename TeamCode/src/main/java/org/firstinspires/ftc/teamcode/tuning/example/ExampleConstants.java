package org.firstinspires.ftc.teamcode.tuning.example;

import com.acmerobotics.dashboard.config.Config;

@Config
public class ExampleConstants {

    public static double CENTER_SERVO_POS = 0.50;
    public static double INTAKE_SERVO_POS = 0.50;
    public static double LEFT_SERVO_POS = 0.50;

    public static double RIGHT_SERVO_POS = 0.50;

    public static double CLAW_SERVO_POS = 0.50;
    public static double WRIST_SERVO_POS = 0.50;

    public static double CENTER_MOTOR_POWER = 0.0;
    public static double CENTER_SERVO_POWER = 0.0;
    public static double LEFT_SERVO_POWER = 0.0;
    public static double RIGHT_SERVO_POWER = 0.0;
}
